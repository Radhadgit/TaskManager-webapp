...
"""
  ...
  stages {
    ...
    /**************************************************************
     * 0 - Declarative: Checkout SCM (Jenkins' built-in checkout)
     *************************************************************/
    ...
    /**************************************************************
     * 1 - Explicit Checkout (same repo; kept because you asked)
     *************************************************************/
    stage('Checkout') {
      steps {
        container('nodejs') {
    ...
    /**************************************************************
     * 3 - Build Docker Image (start dind, wait for docker daemon)
     *************************************************************/
      ...
      steps {
        container('dind') {
          sh '''
            ...
            # give dockerd time to start; try with timeout
            timeout=60
    ...
    /**************************************************************
     * 4 - SonarQube Scan (uses SONAR token stored in Jenkins)
     *    NOTE: This runs npx sonar-scanner (assumes it's available via npx or package)
     *************************************************************/
    stage('SonarQube Scan') {
      steps {
        container('nodejs') {
          withCredentials([string(credentialsId: env.SONAR_CREDENTIAL, variable: 'SONAR_TOKEN')]) {
            // withSonarQubeEnv is available if SonarQube plugin configured with the server name used below
            // we still pass SONAR_TOKEN directly to sonar-scanner
            sh '''
              echo ">>> Running Sonar Scanner against ${SONAR_HOST_URL}"
              # you can use npx sonar-scanner if your project uses sonar-scanner or global install
              npx --no-install sonar-scanner || sonar-scanner || true
              # Fallback: run explicit with properties if sonar-scanner is present
              if command -v sonar-scanner >/dev/null 2>&1; then
                sonar-scanner -Dsonar.projectKey=${REPO_NAME} -Dsonar.sources=. -Dsonar.host.url=${SONAR_HOST_URL} -Dsonar.login=${SONAR_TOKEN}
              else
                echo "sonar-scanner not installed in container; skipping detailed scan (ensure CLI available if required)"
              fi
            '''
          }
        }
    ...
    /**************************************************************
     * 7 - Create Namespace (if not present)
     *************************************************************/
    ...
    /**************************************************************
     * 8 - Create Registry Secret (docker-registry) in K8s
     *************************************************************/
    ...
    /**************************************************************
     * 9 - Create Application Secret (example)
     *************************************************************/
    stage('Create Application Secrets') {
      steps {
        container('kubectl') {
          // Replace these literals with Jenkins credentials if you want them secret in Jenkins
          sh '''
     ...
     *     - Applies k8s manifests and patches deployment image to the new tag
     *************************************************************/
      ...
      steps {
        container('kubectl') {
          sh '''
            ...
            # Patch the deployment image (assumes container name matches REPO_NAME; adjust if different)
    ...
    /**************************************************************
     * 11 - Cleanup (safe; runs in proper container contexts)
     *************************************************************/
    stage('Cleanup') {
      steps {
        script {
          // remove node_modules in nodejs container (best-effort)
          try {
            container('nodejs') {
              sh 'rm -rf node_modules || true'
            }
          } catch (err) {
            echo "Could not remove node_modules inside nodejs container: ${err}"
          }

          // prune docker images inside dind (best-effort)
          try {
            container('dind') {
              sh 'docker system prune -af || true'
            }
          } catch (err) {
            echo "Docker prune skipped or failed: ${err}"
          }

          // delete workspace directory (server-side) - use deleteDir() which requires node context
          // In declarative with kubernetes plugin the node context exists; deleteDir will remove workspace
          deleteDir()
        }
      }
    }
